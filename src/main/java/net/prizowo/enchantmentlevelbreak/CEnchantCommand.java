...
public class CEnchantCommand {
    ...
    private static int enchantItem(CommandSourceStack source, Enchantment enchantment, int level) throws CommandSyntaxException {
        ...
        if (itemStack.isEmpty()) {
            source.sendFailure(new TextComponent("You must be holding an item to enchant"));
            return 0;
        }

        if (enchantment == null) {
            source.sendFailure(new TextComponent("Invalid enchantment"));
            return 0;
        }
        ...
        source.sendSuccess(new TextComponent("Applied " + enchantment.getFullname(level).getString() + " to the item"), true);
